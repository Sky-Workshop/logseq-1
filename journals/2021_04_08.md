...
kik:Adres vph heeft ook de specifiekere klasse vph:AddressDescription. Zie vph:LocalizablePlaceDescription en de subklassen die ik daar nu
heb toegevoegd ten behoeve van alignment met de BAG (bag:postcode, bg:woonplaats, etc). Ik haal deze mogelijk weg in een latere versie VPH.
...
In dit geval wordt nu een subklasse relatie gebruikt voor wat een deel-heel relatie is; bij vph:Organization staat een uitleg over hoe dit anders kan:
“ Note that sub organizations such as 'department' or a 'advisory board' are an Organization as well. For instance, 'Amsterdam Academic Hospital'
is an instance of Organization, more specifically an instance of HospitalOrganization. Their Cardiology Department is an instance of CareOrganization
while the Bookkeeping Department is an instance of e.g. AccountingOrganization.
To define that these instances are part of a larger Organization, we will be using a property such as 'vph:hasSubOrganization'/'vph:isSubOrganizationOf'.
...
subOrganization of the Amsterdam Academic Hospital but may have a different address, while some 'advisory board' shares the address of the hospital
but is not part of the Organization (it is an external Organization).
What should or should not be considered a sub organization specifically within the Nursing Home care domain, and on what grounds, should be determined
and defined outside the ontology. The ontology is deliberately not restricive in this.”
kik:Arbeidsongeschiktheid In VPH gaat dit als een vph:InternalQuality gemodelleerd worden met een overgeerfd “inheresIn only Human” axioma. Daarmee zie ik ‘arbeidsongeschikt’ dus als
een (tijdelijk) kenmerk van de staat van een mens waar je nog een ernst, duur of percentage over kan uitdrukken; equivalent aan bijvoorbeeld ‘ziek’, ‘gehandicapt’, etc.
...
Hier hebben we het eerder over gehad. Hetzelfde probleem gaan we binnenkort ook tegenkomen bij de iStandaarden waar de nadruk overigens meer ligt op
het digitale en fysieke object (vph:ContentBearingObject) en minder op de inhoud (vph:Description of vph:InformationObject).
De kwestie is als volgt: een woord als ‘arbeidscontract’ heeft twee betekenissen die in het dagelijks taalgebruik en ook in data in elkaar overvloeien. Dit is een typisch
voorbeeld van een zogeheten ‘complex type’ en ‘regular polysemy’. De twee betekenissen:
...
Beide bestaan en gaan in de realiteit en dus ook in data eigen en intrinsieke relaties aan met processen en objecten. Het lijkt erop dat we voor de verpleeghuiszorg primair
van alles willen weten over wat er in de contracten staat. Immers, als je iets wil weten over de ‘omvang’ van een contract, ben je niet geïnteresseerd in ‘A4-formaat’ als
antwoord.
...
Kortom, ik probeer binnenkort de relatie tussen Object en Inhoud op een zinnige manier te specificeren zodat het patroon eventueel ook in het Conceptueel Model voor de
iStandaarden kan worden hergebruikt. Zeg iets als: Description: subclassOf/EquivalentClass represented in only ContentBearingObject.
Daarbij: ik weet niet of het handig is alle axioma’s voor deze klasse te definiëren met een existentiële restrictie (some), er zijn bijvoorbeeld Arbeidsovereenkomsten
zonder einddatum en die klasse erft dit axioma nu ook over. (Ik kijk in een latere instantie nog naar de axioma’s in KIK, maar deze viel me nu op.)
kik:BehandelingRondLevenseinde. Lastig en onduidelijk ding in uitvraag wat mij betreft. Strikt genomen is dit de set aan mogelijke behandelingen (zie vph:TherapeuticIntervention / vph:Perdurant) en
acties die kunnen worden toegepast (zo’n beetje alles natuurlijk) en waar men nu wat vage begrippen bij heeft gezet die met een boolean mogen worden beantwoord.
Tja.
...
Strikt genomen zijn de genoemde instanties part-of de BehandelingRondLevensEindeBeschrijving; zie ook het voorstel bij kik:Contractomvang hieronder.
